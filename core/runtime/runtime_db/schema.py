...
async def ensure_graph_runs_tables(conn: Any) -> None:
    async with conn.cursor() as cursor:
        await cursor.execute(
            """
        ...
        await cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_graph_runs_graph_updated_at
                ON graph_runs (graph_name, updated_at DESC)
            """
        )
        await cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_graph_runs_service_updated_at
                ON graph_runs (service_name, updated_at DESC)
            """
        )
        await cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_graph_runs_status_updated_at
        ...
        await cursor.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_graph_node_runs_run_id
                ON graph_node_runs (run_id, created_at)
            """
        )
    await conn.commit()
