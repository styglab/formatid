...
class PpsObjectStorage:
    def __init__(self) -> None:
        settings = get_settings()
        if not settings.s3_endpoint:
            raise RuntimeError("PPS_S3_ENDPOINT is not configured")
        if not settings.s3_access_key or not settings.s3_secret_key:
            raise RuntimeError("PPS_S3_ACCESS_KEY/PPS_S3_SECRET_KEY are not configured")
        if not settings.s3_bucket:
            raise RuntimeError("PPS_S3_BUCKET is not configured")
...
def _normalize_endpoint(raw_endpoint: str, *, default_secure: bool) -> tuple[str, bool]:
    ...
    if not endpoint:
        raise RuntimeError(f"invalid PPS_S3_ENDPOINT: {raw_endpoint}")
