...
import { semanticAdminRequest } from "@/api/semantic-admin";
...
type ExecutionContractsTab = "sources" | "assets" | "operations";
type RegistryInspectorView =
  | "semantic"
  | "relationship"
  | "create-relationship"
  | "create-semantic";
type RegistryInspectorTab = "overview" | "relations" | "history" | "edit";
type DrawerKind =
  | "create"
  | "semantic"
  | "relationship"
...
const CAPABILITIES_PAGE_SIZE = 10;
...
const mappingDefaults: MappingForm = {
  sourceId: "",
  operationId: "",
  fieldPath: "",
  semanticTypeId: "",
...
export default function Page() {
  ...
  const [executionAssets, setExecutionAssets] = useState<ExecutionAsset[]>([]);
  const [executionOperations, setExecutionOperations] = useState<ExecutionOperation[]>([]);
  const [operationFields, setOperationFields] = useState<OperationField[]>([]);
  ...
  const [activeModule, setActiveModule] = useState<ModuleView>("semantic-types");
  const [activeTab, setActiveTab] = useState<SemanticTab>("registry");
  const [activeExecutionTab, setActiveExecutionTab] = useState<ExecutionContractsTab>("sources");
  const [registryInspectorView, setRegistryInspectorView] = useState<RegistryInspectorView>("semantic");
  const [registryInspectorTab, setRegistryInspectorTab] = useState<RegistryInspectorTab>("overview");
  const [currentSemanticRegistryPage, setCurrentSemanticRegistryPage] = useState(1);
  const [currentTypesPage, setCurrentTypesPage] = useState(1);
  const [currentAttributesPage, setCurrentAttributesPage] = useState(1);
  const [currentRelationshipsPage, setCurrentRelationshipsPage] = useState(1);
  ...
  const [currentCapabilitiesPage, setCurrentCapabilitiesPage] = useState(1);
  const [currentMappingsPage, setCurrentMappingsPage] = useState(1);
  ...
  const [selectedExecutionAssetId, setSelectedExecutionAssetId] = useState("");
  const [selectedExecutionOperationId, setSelectedExecutionOperationId] = useState("");
  ...
  const [mappingForm, setMappingForm] = useState<MappingForm>(mappingDefaults);
  const [mappingEditorMode, setMappingEditorMode] = useState<"detail" | "create">("detail");
  ...
  useEffect(() => {
    setCurrentSemanticRegistryPage(1);
  ...
  const selectedExecutionAsset = useMemo(
    () => executionAssets.find((item) => item.id === selectedExecutionAssetId) || null,
    [executionAssets, selectedExecutionAssetId]
  );
  const selectedExecutionOperation = useMemo(
    () => executionOperations.find((item) => item.id === selectedExecutionOperationId) || null,
    [executionOperations, selectedExecutionOperationId]
  );
  ...
  const selectedMapping = useMemo(
    () => fieldMappings.find((item) => item.id === selectedMappingId) || null,
    [fieldMappings, selectedMappingId]
  );
  useEffect(() => {
    if (selectedMapping && mappingEditorMode === "detail") {
      fillMappingForm(selectedMapping);
    }
  }, [mappingEditorMode, selectedMapping]);
  const operationFieldsForSelection = useMemo(
    () => operationFields.filter((item) => item.operation_id === selectedExecutionOperationId),
    [operationFields, selectedExecutionOperationId]
  );
  const canonicalAttributeOptions = useMemo(
    () =>
      semanticTypes.filter((item) => {
        const display = item.draft_snapshot || item;
        return display.entity_kind === "attribute";
      }),
    [semanticTypes]
  );
  ...
  useEffect(() => {
    setCurrentRelationshipsPage(1);
  }, [selectedEntityContextId, statusFilter, searchQuery]);
  ...
  useEffect(() => {
    setCurrentCapabilitiesPage(1);
    setCurrentMappingsPage(1);
  }, [searchQuery, statusFilter]);
  ...
  const filteredSemanticRegistry = useMemo(() => {
    const lowered = searchQuery.trim().toLowerCase();
    return semanticTypes.filter((item) => {
      ...
      const searchMatches =
        ...
        (display.aliases || []).some((alias) => alias.toLowerCase().includes(lowered));
      return statusMatches && searchMatches;
    });
  }, [searchQuery, semanticTypes, statusFilter]);
  ...
  const totalSemanticRegistryPages = Math.max(1, Math.ceil(filteredSemanticRegistry.length / TYPES_PAGE_SIZE));
  const paginatedSemanticRegistry = useMemo(() => {
    const start = (currentSemanticRegistryPage - 1) * TYPES_PAGE_SIZE;
    return filteredSemanticRegistry.slice(start, start + TYPES_PAGE_SIZE);
  }, [currentSemanticRegistryPage, filteredSemanticRegistry]);
  ...
  const attributeCountMap = useMemo(() => {
    const counts = new Map<string, number>();
    for (const attribute of attributeSemanticTypes) {
      const display = attribute.draft_snapshot || attribute;
      const hasPending = hasPendingOverlay(attribute);
      const include =
        statusFilter === "all"
          ? true
          : statusFilter === "approved"
            ? !isDraftCreate(attribute)
            : hasPending;
      if (include && display.parent_entity_id) {
        counts.set(display.parent_entity_id, (counts.get(display.parent_entity_id) || 0) + 1);
      }
    }
    return counts;
  }, [attributeSemanticTypes, statusFilter]);
  ...
  const selectedSemanticAttributes = useMemo(() => {
    ...
    return semanticTypes.filter((item) => {
      const display = item.draft_snapshot || item;
      if (!(display.entity_kind === "attribute" && display.parent_entity_id === selectedEntityContextId)) {
        return false;
      }
      const hasPending = hasPendingOverlay(item);
      if (statusFilter === "approved") {
  ...
  }, [selectedEntityContextId, semanticTypes, statusFilter]);

  const outgoingRelationships = useMemo(
    () => visibleRelationships.filter((item) => item.source_id === selectedEntityContextId),
    [selectedEntityContextId, visibleRelationships]
  );
  const incomingRelationships = useMemo(
    () => visibleRelationships.filter((item) => item.target_id === selectedEntityContextId),
    [selectedEntityContextId, visibleRelationships]
  );

  const relatedPendingProposals = useMemo(() => {
    const focusId = selectedRelationshipId || selectedSemanticTypeId || selectedEntityContextId;
    if (!focusId) {
      return [];
    }
    return proposals.filter((proposal) => proposal.entity_id === focusId);
  }, [proposals, selectedEntityContextId, selectedRelationshipId, selectedSemanticTypeId]);
  ...
  const relationshipTableItems = useMemo(() => {
    if (!selectedEntityContextId) {
      return [];
    }
    return filteredRelationshipsByView.filter(
      (item) => item.source_id === selectedEntityContextId || item.target_id === selectedEntityContextId
    );
  }, [filteredRelationshipsByView, selectedEntityContextId]);

  const totalRelationshipPages = Math.max(1, Math.ceil(relationshipTableItems.length / ATTRIBUTES_PAGE_SIZE));
  const paginatedRelationships = useMemo(() => {
    const start = (currentRelationshipsPage - 1) * ATTRIBUTES_PAGE_SIZE;
    return relationshipTableItems.slice(start, start + ATTRIBUTES_PAGE_SIZE);
  }, [currentRelationshipsPage, relationshipTableItems]);
  ...
  const filteredExecutionAssets = useMemo(() => {
    const lowered = sourceQuery.trim().toLowerCase();
    return executionAssets.filter((item) => {
      const statusMatches =
        statusFilter === "all"
          ? true
          : statusFilter === "approved"
      ...
      const searchMatches =
        !lowered ||
        item.name.toLowerCase().includes(lowered) ||
        item.locator.toLowerCase().includes(lowered) ||
        (item.source_name || "").toLowerCase().includes(lowered);
      return statusMatches && searchMatches;
    });
  }, [executionAssets, sourceQuery, statusFilter]);

  const filteredExecutionOperations = useMemo(() => {
    const lowered = sourceQuery.trim().toLowerCase();
    return executionOperations.filter((item) => {
      const statusMatches =
        statusFilter === "all"
          ? true
          : statusFilter === "approved"
      ...
      const searchMatches =
        ...
        (item.source_name || "").toLowerCase().includes(lowered) ||
        (item.asset_name || "").toLowerCase().includes(lowered);
      return statusMatches && searchMatches;
    });
  }, [executionOperations, sourceQuery, statusFilter]);
  ...
  const filteredCapabilities = useMemo(() => {
    ...
    return capabilities.filter((item) => {
      ...
      const searchMatches =
        !lowered ||
        display.name.toLowerCase().includes(lowered) ||
        display.capability_key.toLowerCase().includes(lowered) ||
        (display.description || "").toLowerCase().includes(lowered);
      return statusMatches && searchMatches;
    });
  }, [capabilities, searchQuery, statusFilter]);
  ...
  const filteredMappings = useMemo(() => {
    ...
    return fieldMappings.filter((item) => {
      ...
      const searchMatches =
        !lowered ||
        display.operation_id.toLowerCase().includes(lowered) ||
        display.field_path.toLowerCase().includes(lowered) ||
        display.semantic_type_id.toLowerCase().includes(lowered) ||
        (display.canonical_attribute_id || "").toLowerCase().includes(lowered);
      return statusMatches && searchMatches;
    });
  }, [fieldMappings, searchQuery, statusFilter]);
  ...
  const totalAssetsPages = Math.max(1, Math.ceil(filteredExecutionAssets.length / SOURCES_PAGE_SIZE));
  const paginatedExecutionAssets = useMemo(() => {
    const start = (currentAssetsPage - 1) * SOURCES_PAGE_SIZE;
    return filteredExecutionAssets.slice(start, start + SOURCES_PAGE_SIZE);
  }, [currentAssetsPage, filteredExecutionAssets]);
  const totalOperationsPages = Math.max(1, Math.ceil(filteredExecutionOperations.length / SOURCES_PAGE_SIZE));
  const paginatedExecutionOperations = useMemo(() => {
    const start = (currentOperationsPage - 1) * SOURCES_PAGE_SIZE;
    return filteredExecutionOperations.slice(start, start + SOURCES_PAGE_SIZE);
  }, [currentOperationsPage, filteredExecutionOperations]);

  const totalCapabilitiesPages = Math.max(1, Math.ceil(filteredCapabilities.length / CAPABILITIES_PAGE_SIZE));
  const paginatedCapabilities = useMemo(() => {
    const start = (currentCapabilitiesPage - 1) * CAPABILITIES_PAGE_SIZE;
    return filteredCapabilities.slice(start, start + CAPABILITIES_PAGE_SIZE);
  }, [currentCapabilitiesPage, filteredCapabilities]);

  const totalMappingsPages = Math.max(1, Math.ceil(filteredMappings.length / MAPPINGS_PAGE_SIZE));
  const paginatedMappings = useMemo(() => {
    const start = (currentMappingsPage - 1) * MAPPINGS_PAGE_SIZE;
    return filteredMappings.slice(start, start + MAPPINGS_PAGE_SIZE);
  }, [currentMappingsPage, filteredMappings]);
  ...
  async function loadData() {
    ...
    try {
      const [
        ...
        assetsResponse,
        operationsResponse,
        operationFieldsResponse,
        capabilitiesResponse,
        mappingsResponse,
        relationshipsResponse,
        proposalsResponse
      ] = await Promise.all([
        semanticAdminRequest<Overview>("/api/overview"),
        semanticAdminRequest<SemanticType[]>("/api/semantic-types"),
        semanticAdminRequest<ExecutionSource[]>("/api/execution-sources"),
        semanticAdminRequest<ExecutionAsset[]>("/api/execution-assets"),
        semanticAdminRequest<ExecutionOperation[]>("/api/execution-operations"),
        semanticAdminRequest<OperationField[]>("/api/operation-fields"),
        semanticAdminRequest<Capability[]>("/api/capabilities"),
        semanticAdminRequest<FieldMapping[]>("/api/mappings"),
        semanticAdminRequest<SemanticRelationship[]>("/api/semantic-relationships"),
        semanticAdminRequest<Proposal[]>("/api/proposals?status=pending_review")
      ]);
      setOverview(overviewResponse);
      setSemanticTypes(typesResponse);
      setExecutionSources(sourcesResponse);
      setExecutionAssets(assetsResponse);
      setExecutionOperations(operationsResponse);
      setOperationFields(operationFieldsResponse);
      setCapabilities(capabilitiesResponse);
      setFieldMappings(mappingsResponse);
      ...
      if (assetsResponse.length) {
        setSelectedExecutionAssetId((current) =>
          assetsResponse.some((item) => item.id === current) ? current : assetsResponse[0].id
        );
      } else {
        setSelectedExecutionAssetId("");
      }

      if (operationsResponse.length) {
        setSelectedExecutionOperationId((current) =>
          operationsResponse.some((item) => item.id === current) ? current : operationsResponse[0].id
        );
      } else {
        setSelectedExecutionOperationId("");
      }

      if (capabilitiesResponse.length) {
        setSelectedCapabilityId((current) =>
          capabilitiesResponse.some((item) => item.id === current) ? current : capabilitiesResponse[0].id
        );
      } else {
        setSelectedCapabilityId("");
      }

      if (mappingsResponse.length) {
        setSelectedMappingId((current) =>
          mappingsResponse.some((item) => item.id === current) ? current : mappingsResponse[0].id
        );
      } else {
      ...
      return {
        ...
        executionAssets: assetsResponse,
        executionOperations: operationsResponse,
  ...
  function fillMappingForm(mapping: FieldMapping) {
    ...
    setMappingForm({
      ...
      notes: display.notes || "",
      transformSpec: JSON.stringify(display.transform_spec || {}, null, 2),
      enumMapping: JSON.stringify(display.enum_mapping || {}, null, 2)
    });
  }
  ...
  function selectSemanticType(semanticType: SemanticType, options?: { openDrawer?: boolean }) {
    ...
    setRegistryInspectorView("semantic");
    setRegistryInspectorTab("overview");
  ...
  function selectRelationship(relationship: SemanticRelationship, options?: { openDrawer?: boolean }) {
    ...
    setRegistryInspectorView("relationship");
    setRegistryInspectorTab("relations");
  ...
  function selectExecutionAsset(asset: ExecutionAsset) {
    setSelectedExecutionAssetId(asset.id);
  }

  function selectExecutionOperation(operation: ExecutionOperation) {
    setSelectedExecutionOperationId(operation.id);
  }

  function selectMapping(mapping: FieldMapping) {
    setSelectedMappingId(mapping.id);
    fillMappingForm(mapping);
    setMappingEditorMode("detail");
  }
  ...
  function openCreateEntityDrawer() {
    ...
    setSelectedSemanticTypeId("");
    setSelectedRelationshipId("");
    setSemanticTypeForm(semanticTypeDefaults);
    setRelationshipEditForm(relationshipDefaults);
    setRegistryInspectorView("create-semantic");
    setRegistryInspectorTab("edit");
  }
  ...
  function openCreateMapping() {
    if (!canEditCurrentView) return;
    setSelectedMappingId("");
    setMappingEditorMode("create");
    setMappingForm({
      ...mappingDefaults,
      sourceId: selectedExecutionSourceId,
      operationId: selectedExecutionOperationId
    });
  }
  ...
  async function submitCreateSemanticType(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await semanticAdminRequest("/api/semantic-types", {
        method: "POST",
  ...
  async function submitSemanticType(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await semanticAdminRequest(`/api/semantic-types/${selectedSemanticTypeId}`, {
        method: "PATCH",
        body: JSON.stringify({
  ...
  async function submitRelationship(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await semanticAdminRequest(`/api/semantic-types/${relationshipForm.sourceId}/relationships`, {
        method: "POST",
        body: JSON.stringify({
  ...
  async function submitCreateExecutionSource(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await semanticAdminRequest("/api/execution-sources", {
        method: "POST",
        body: JSON.stringify(payload)
      });
      const refreshed = await loadData();
  ...
  async function submitExecutionSource(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await semanticAdminRequest(`/api/execution-sources/${selectedExecutionSourceId}`, {
        method: "PATCH",
  ...
  async function submitRelationshipEdit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await semanticAdminRequest(`/api/semantic-relationships/${selectedRelationshipId}`, {
        method: "PATCH",
        body: JSON.stringify({
  ...
  async function submitMapping(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setSubmitting(mappingEditorMode === "create" ? "create-mapping" : "mapping");
    try {
      const transformSpec = parseJsonObject(mappingForm.transformSpec, "transform_spec");
      const enumMapping = parseJsonObject(mappingForm.enumMapping, "enum_mapping");
      const payload = {
        source_id: mappingForm.sourceId || null,
        operation_id: mappingForm.operationId.trim(),
        field_path: mappingForm.fieldPath.trim(),
        semantic_type_id: mappingForm.semanticTypeId.trim(),
        canonical_attribute_id: mappingForm.canonicalAttributeId || null,
        mapping_type: mappingForm.mappingType,
        mapping_kind: mappingForm.mappingKind,
        namespace: mappingForm.namespace.trim() || "public",
        lifecycle: mappingForm.lifecycle.trim() || "draft",
        version: mappingForm.version.trim() || "1.0.0",
        confidence: mappingForm.confidence.trim() ? Number(mappingForm.confidence) : null,
        notes: mappingForm.notes.trim(),
        transform_spec: transformSpec,
        enum_mapping: enumMapping
      };
      if (mappingEditorMode === "create") {
        await semanticAdminRequest("/api/mappings", {
          method: "POST",
          body: JSON.stringify(payload)
        });
      } else if (selectedMappingId) {
        await semanticAdminRequest(`/api/mappings/${selectedMappingId}`, {
          method: "PATCH",
          body: JSON.stringify(payload)
        });
      }
      const refreshed = await loadData();
      if (mappingEditorMode === "create") {
        const created = refreshed?.mappings.find(
          (item) => item.operation_id === payload.operation_id && item.field_path === payload.field_path
        );
        if (created) {
      ...
      setMappingEditorMode("detail");
      showToast("success", mappingEditorMode === "create" ? "mapping created" : "mapping updated");
    } catch (requestError) {
      showToast("error", requestError instanceof Error ? requestError.message : "Failed to save mapping.");
    } finally {
      setSubmitting("");
    }
  }
  ...
  async function deleteMapping() {
    if (!selectedMappingId || !window.confirm("This will create a delete proposal for the selected mapping. Continue?")) {
      return;
    }
    setSubmitting("delete-mapping");
    try {
      await semanticAdminRequest(`/api/mappings/${selectedMappingId}`, { method: "DELETE" });
      setSelectedMappingId("");
      await loadData();
      showToast("success", "mapping delete proposal created");
    } catch (requestError) {
      showToast("error", requestError instanceof Error ? requestError.message : "Failed to delete mapping.");
    } finally {
      setSubmitting("");
    }
  }
  ...
  async function reviewProposal(proposalId: string, decision: "approve" | "reject") {
    ...
    try {
      await semanticAdminRequest(`/api/proposals/${proposalId}/${decision}`, {
        method: "POST",
  ...
  async function deleteSemanticType() {
    ...
    try {
      await semanticAdminRequest(`/api/semantic-types/${selectedSemanticTypeId}`, { method: "DELETE" });
      setSelectedSemanticTypeId("");
      setOpenDrawer(null);
      await loadData();
  ...
  async function deleteRelationship() {
    ...
    try {
      await semanticAdminRequest(`/api/semantic-relationships/${selectedRelationshipId}`, { method: "DELETE" });
      setSelectedRelationshipId("");
      setOpenDrawer(null);
      setRegistryInspectorView("semantic");
      await loadData();
  ...
  async function deleteExecutionSource() {
    ...
    try {
      await semanticAdminRequest(`/api/execution-sources/${selectedExecutionSourceId}`, { method: "DELETE" });
      setSelectedExecutionSourceId("");
      setOpenDrawer(null);
      await loadData();
      ...
      <div className="flex min-h-screen bg-background text-foreground">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <main className="mx-auto flex w-full max-w-[1680px] flex-1 flex-col p-4 lg:p-5">
            <section className="mb-3 rounded-2xl border border-amber-500/20 bg-amber-500/8 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
              This route is the legacy prototype preserved for migration reference. Active dashboard work has moved to
              `/sources`, `/onboarding-runs`, `/source-operations`, `/schemas`, `/semantic-types`, `/canonical-model`, `/mappings`, `/lineage`, `/capabilities`, `/operations`, `/variants`, `/bindings`, `/proposal-bundles`, `/proposals`, `/reviews`, `/audit`, and `/publish`.
            </section>
            <section className="mb-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-sm">
                <h1 className="text-lg font-semibold tracking-tight">
                  {activeModule === "semantic-types"
                    ? "Semantic Types"
                    : activeModule === "canonical-model"
                      ? labels.title
                    : activeModule === "mappings"
                      ? "Mappings"
                ...
                {activeModule === "semantic-types" ? (
                  <>
                    <InlineStat label="Types" value={semanticTypes.length} />
                ...
                {activeModule === "mappings" ? (
                  <>
                    <InlineStat label="Mappings" value={fieldMappings.length} />
                    <InlineStat label="Pending" value={overview?.counts.pending_proposals ?? 0} />
                  </>
                ) : null}
                {activeModule === "capabilities" ? (
                  <>
                    <InlineStat label="Capabilities" value={capabilities.length} />
                    <InlineStat label="Pending" value={overview?.counts.pending_proposals ?? 0} />
                  </>
                ) : null}
                {activeModule === "operations" ? (
                ...
                {activeModule === "governance" ? (
                  <>
                    <InlineStat label="Pending Reviews" value={overview?.counts.pending_proposals ?? 0} />
                    <InlineStat label="Recent" value={proposals.length} />
                  </>
                ) : null}
              </div>
            </section>
            ...
            {activeModule === "semantic-types" ? (
              <section className="space-y-4">
                <div className="grid min-h-0 gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
                  <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                    <CardHeader className="border-b border-border p-4">
                      <div className="flex min-w-0 items-center justify-between gap-3">
                        <div className="min-w-0">
                          <CardTitle>Browse</CardTitle>
                          <div className="mt-1 truncate text-xs text-muted-foreground">Canonical meaning units used across entities, mappings, and capabilities</div>
                        </div>
                        <Badge variant="info">{filteredSemanticRegistry.length}</Badge>
                      </div>
                    </CardHeader>
                    <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                      ...
                      ) : filteredSemanticRegistry.length ? (
                        <>
                          <div className="flex-1 divide-y divide-border/70 overflow-auto">
                            {paginatedSemanticRegistry.map((semanticType) => {
                              const display = semanticType.draft_snapshot || semanticType;
                              const isSelected = semanticType.id === selectedSemanticTypeId;
                              return (
                                <button
                                  key={semanticType.id}
                                  type="button"
                                ...
                                >
                                  <div className="flex min-w-0 items-start justify-between gap-3">
                                    <div className="min-w-0 flex-1">
                                      <div className="truncate text-[13px] font-medium">{display.name}</div>
                                      <div className="mt-1 truncate text-[11px] text-muted-foreground">
                                        {(display.entity_kind || "semantic").toUpperCase()} · {display.parent_entity_name || (display.aliases || []).slice(0, 2).join(", ") || display.id}
                                      </div>
                                    </div>
                          ...
                          <PaginationBar
                            currentPage={currentSemanticRegistryPage}
                            totalPages={totalSemanticRegistryPages}
                            onPageChange={setCurrentSemanticRegistryPage}
                          />
                        </>
                      ) : (
                        <div className="p-4">
                        <EmptyState icon={Fingerprint} label="No semantic types yet." />
                      </div>
                    )}
                  </CardContent>
                  </Card>
                  ...
                  <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                    <CardHeader className="border-b border-border p-4">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">Semantic type overview, aliases, canonical links, and governance status</div>
                      </div>
                    </CardHeader>
                        ...
                        <>
                          <div className="space-y-3">
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <div className="text-lg font-semibold">{selectedSemanticDisplay?.name}</div>
                                <div className="mt-1 text-xs text-muted-foreground">
                                  {(selectedSemanticDisplay?.entity_kind || "semantic").toUpperCase()} · {selectedSemanticDisplay?.datatype || "string"}
                                </div>
                              </div>
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                              ...
                              <MetaPill label="Kind" value={selectedSemanticDisplay?.entity_kind || "semantic"} />
                              <MetaPill label="Datatype" value={selectedSemanticDisplay?.datatype || "string"} />
                            </div>
                          </div>
                          {selectedSemanticDisplay?.description ? (
                            <div className="rounded-xl border border-border/70 bg-muted/15 p-4 text-sm leading-6">
                              {selectedSemanticDisplay.description}
                            </div>
                          ) : null}
                          <InspectorList label="Aliases" values={selectedSemanticDisplay?.aliases || []} emptyLabel="No aliases" />
                          <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
                            <div className="mb-3 text-xs uppercase tracking-[0.14em] text-muted-foreground">Canonical Links</div>
                            <div className="space-y-2 text-sm">
                              <div>Parent Entity: {selectedSemanticDisplay?.parent_entity_name || "-"}</div>
                              <div>Owners: {(selectedSemanticDisplay?.owners || []).join(", ") || "platform"}</div>
                            </div>
                          </div>
                        </>
                      )}
            ...
            {activeModule === "canonical-model" && activeTab === "registry" ? (
              <section className="space-y-4">
                <div className="grid min-h-0 gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="relative border-b border-border p-4 pr-28">
                    <div>
                      <CardTitle>Browse</CardTitle>
                      <div className="mt-1 text-xs text-muted-foreground">Entities and attributes in the current semantic model</div>
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
                  ...
                  <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                    {loading ? (
                      ...
                      <>
                        <div className="border-b border-border/70 bg-muted/10 px-4 py-2.5">
                          <div className="flex items-center justify-between gap-3 text-[11px] text-muted-foreground">
                            <span>{filteredSemanticTypes.length} entities</span>
                            <span>{attributeSemanticTypes.length} attributes</span>
                          </div>
                        </div>
                        <div className="flex-1 divide-y divide-border/70 overflow-auto">
                          {paginatedSemanticTypes.map((semanticType) => {
                            const displaySemanticType = semanticType.draft_snapshot || semanticType;
                            const treeAttributes = attributeSemanticTypes.filter((attribute) => {
                              ...
                              const matchesStatus =
                                statusFilter === "all"
                                  ...
                                  : statusFilter === "approved"
                                    ? !isDraftCreate(attribute)
                                    : hasPending;
                              return matchesStatus && displayAttribute.parent_entity_id === semanticType.id;
                            });
                            const isSelected = semanticType.id === selectedEntityContextId;
                            const expanded = expandedEntityIds.includes(semanticType.id);
                            return (
                              <div key={semanticType.id} className={isSelected ? "bg-primary/5" : ""}>
                                <div className="group flex items-start gap-2 px-4 py-2.5">
                                  <button
                                    ...
                                    className="mt-0.5 rounded-md p-1 text-muted-foreground transition hover:bg-accent hover:text-foreground"
                                  >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => selectSemanticType(semanticType)}
                                    className="min-w-0 flex-1 rounded-xl border border-transparent px-2 py-1 text-left transition group-hover:bg-muted/20"
                                  >
                                    <div className="flex items-start justify-between gap-3">
                                      <div className="min-w-0">
                                        ...
                                        <div className="mt-1 flex flex-wrap items-center gap-1.5">
                                          <span className="rounded-full border border-border/70 bg-background px-2 py-0.5 text-[10px] text-muted-foreground">
                                            {attributeCountMap.get(semanticType.id) || 0} attributes
                                          </span>
                                          <span className="rounded-full border border-border/70 bg-background px-2 py-0.5 text-[10px] text-muted-foreground">
                                      ...
                                      <div className="flex shrink-0 flex-col items-end gap-1">
                                        {semanticType.draft_snapshot ? <Badge className="px-1.5 py-0 text-[10px]" variant="warning">Draft</Badge> : null}
                                      </div>
                                    </div>
                                  </button>
                                </div>
                                {expanded ? (
                                  <div className="border-t border-border/50 bg-muted/10 px-4 py-2">
                                    <div className="mb-2 flex items-center justify-between gap-2">
                                      <div className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Attributes</div>
                                      <Button
                                        size="sm"
                                        variant="ghost"
                                        className="h-6 px-2 text-[10px]"
                                        onClick={() => openCreateAttributeDrawer(semanticType.id)}
                                        disabled={!canEditCurrentView}
                                      >
                                    ...
                                    {treeAttributes.map((attribute) => {
                                        const displayAttribute = attribute.draft_snapshot || attribute;
                                        const isAttributeSelected = attribute.id === selectedSemanticTypeId;
                                        return (
                                          <button
                                            key={attribute.id}
                                            type="button"
                                            onClick={() => selectSemanticType(attribute)}
                                            className={`block w-full rounded-lg border px-3 py-2 text-left transition hover:bg-muted/20 ${
                                              isAttributeSelected
                                          ...
                                          >
                                            <div className="flex items-start justify-between gap-3">
                                              <div className="min-w-0">
                                                <div className="truncate text-[12px] font-medium">{displayAttribute.name}</div>
                                              </div>
                                              {attribute.draft_snapshot ? (
                                    ...
                                    {!treeAttributes.length ? (
                                      <div className="rounded-lg border border-dashed border-border/70 px-3 py-3 text-[11px] text-muted-foreground">
                                        No attributes
                                      </div>
                                    ) : null}
                                  </div>
                ...
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <CardTitle>{labels.sections.inspector}</CardTitle>
                        <div className="mt-1 text-xs text-muted-foreground">Selected semantic object details and governance actions</div>
                      </div>
                      {registryInspectorView === "semantic" || registryInspectorView === "relationship" ? (
                        <div className="inline-flex rounded-lg border border-border bg-muted/20 p-1">
                          {([
                            ...
                            <button
                              key={tab.id}
                              type="button"
                              onClick={() => setRegistryInspectorTab(tab.id)}
                              className={`rounded-md px-3 py-1.5 text-sm transition ${
                                registryInspectorTab === tab.id
                                  ? "bg-primary text-primary-foreground shadow-sm"
                                  : "text-muted-foreground hover:bg-accent hover:text-foreground"
                              }`}
                            >
                  ...
                  <CardContent className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                    {registryInspectorView === "create-semantic" ? (
                      <>
                        <div className="space-y-3">
                          <div className="text-lg font-semibold">
                            {createSemanticTypeForm.entityKind === "attribute" ? labels.actions.createAttribute : labels.actions.createEntity}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {createSemanticTypeForm.entityKind === "attribute"
                              ? "Create an attribute under the selected entity."
                    ...
                    ) : registryInspectorView === "create-relationship" ? (
                      <>
                        <div className="space-y-3">
                          <div className="text-lg font-semibold">Create Relationship</div>
                          <div className="text-xs text-muted-foreground">
                            Define a directed relationship between two entities.
                          </div>
                        </div>
                    ...
                    ) : registryInspectorView === "relationship" && selectedRelationship ? (
                      <>
                        <div className="space-y-3">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <div className="text-lg font-semibold">{selectedRelationship.relation_type}</div>
                              <div className="mt-1 text-xs text-muted-foreground">
                                {selectedRelationship.source_name} → {selectedRelationship.target_name}
                              </div>
                            </div>
                            <div className="flex flex-wrap gap-2">
                              <Badge
                                variant={
                                  selectedRelationship.draft_change_type === "delete"
                                    ? "danger"
                                    : selectedRelationship.draft_snapshot || selectedRelationship.status === "draft"
                                      ? "warning"
                                      : "success"
                                }
                              >
                          ...
                          <div className="grid gap-2 sm:grid-cols-2">
                            ...
                            <MetaPill label="Updated" value={formatDate(selectedRelationship.updated_at, language)} />
                            <MetaPill label="Source" value={selectedRelationship.source_name} />
                            <MetaPill label="Target" value={selectedRelationship.target_name} />
                          </div>
                        </div>
                        ...
                        <form className="space-y-4 border-t border-border pt-4" onSubmit={submitRelationshipEdit}>
                          ...
                          <div className="flex flex-wrap gap-2 border-t border-border pt-4">
                            <Button type="submit" disabled={!canEditCurrentView || submitting === "relationship-edit"}>
                            ...
                            <Button
                              ...
                              disabled={!canEditCurrentView || submitting === "delete-relationship"}
                            >
                    ...
                    ) : registryInspectorView === "relationship" && registryInspectorTab === "overview" && selectedRelationship ? (
                      <>
                        <div className="space-y-3">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                    ...
                    ) : registryInspectorView === "relationship" && registryInspectorTab === "history" && selectedRelationship ? (
                      <>
                        <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
                          <div className="mb-3 text-xs uppercase tracking-[0.14em] text-muted-foreground">History</div>
                          <div className="space-y-2 text-sm">
                            <div>Created: {formatDate(selectedRelationship.created_at, language)}</div>
                            <div>Updated: {formatDate(selectedRelationship.updated_at, language)}</div>
                            <div>Proposal: {selectedRelationship.pending_proposal_id || "-"}</div>
                          </div>
                        </div>
                        {selectedRelationship.draft_snapshot ? (
                          <RelationshipCompareCard
                    ...
                    ) : registryInspectorView === "relationship" && registryInspectorTab === "edit" && selectedRelationship ? (
                      <>
                      ...
                      <EmptyState icon={Eye} label={labels.status.emptySelection} />
                    ) : registryInspectorTab === "relations" && selectedSemanticDisplay?.entity_kind === "entity" ? (
                      <>
                        <div className="space-y-4">
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <div className="text-lg font-semibold">{selectedSemanticDisplay.name}</div>
                              <div className="mt-1 text-xs text-muted-foreground">Relationships anchored to this entity</div>
                            </div>
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => {
                                if (!canEditCurrentView || entitySemanticTypes.length < 2) return;
                                setSelectedRelationshipId("");
                                setSemanticTypeForm(semanticTypeDefaults);
                                setRelationshipEditForm(relationshipDefaults);
                                setRegistryInspectorView("create-relationship");
                                setRelationshipForm({
                                  sourceId: selectedEntityContextId || entitySemanticTypes[0]?.id || "",
                                  targetId: entitySemanticTypes.find((item) => item.id !== selectedEntityContextId)?.id || "",
                                  relationType: "related_to"
                                });
                              }}
                              disabled={!canEditCurrentView || entitySemanticTypes.length < 2}
                            >
                              <Plus className="h-4 w-4" />
                              {labels.actions.createRelation}
                            </Button>
                          </div>
                          <RelationshipDirectionSection
                    ...
                    ) : registryInspectorTab === "history" ? (
                      <>
                        <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
                          <div className="mb-3 text-xs uppercase tracking-[0.14em] text-muted-foreground">History</div>
                          <div className="space-y-2 text-sm">
                            <div>Created: {formatDate(selectedSemanticType.created_at, language)}</div>
                            <div>Updated: {formatDate(selectedSemanticType.updated_at, language)}</div>
                            <div>Proposal: {selectedSemanticType.pending_proposal_id || "-"}</div>
                          </div>
                        </div>
                        {relatedPendingProposals.length ? (
                          <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
                            <div className="mb-3 text-xs uppercase tracking-[0.14em] text-muted-foreground">Open Reviews</div>
                            <div className="space-y-2">
                              {relatedPendingProposals.map((proposal) => (
                                <div key={proposal.id} className="rounded-lg border border-border/70 bg-background px-3 py-2">
                                  <div className="text-sm font-medium">{proposal.title}</div>
                                  <div className="mt-1 text-[11px] text-muted-foreground">
                                    {proposal.change_type} · {formatDate(proposal.created_at, language)}
                                  </div>
                                </div>
                              ))}
                            </div>
                      ...
                      <>
                        <form className="space-y-4" onSubmit={submitSemanticType}>
                          ...
                          />
                          <div className="flex flex-wrap gap-2 border-t border-border pt-4">
                            <Button type="submit" disabled={!canEditCurrentView || submitting === "semantic-type"}>
                      ...
                      (() => {
                        ...
                        return (
                          <>
                            ...
                            <div className="flex flex-wrap gap-2 border-t border-border pt-4">
                              <Button onClick={() => setRegistryInspectorTab("edit")} disabled={!canEditCurrentView}>
                                <PencilLine className="h-4 w-4" />
                                Edit
                              </Button>
                              {displaySemanticType.entity_kind === "entity" ? (
                                <Button variant="outline" onClick={() => setRegistryInspectorTab("relations")}>
                                  <GitBranch className="h-4 w-4" />
                                  Relations
                                </Button>
                              ) : null}
            ...
            {activeModule === "canonical-model" && activeTab === "graph" ? (
              <section className="space-y-4">
                <Card className="workspace-panel overflow-hidden rounded-2xl shadow-none">
              ...
              <section className="grid min-h-0 gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <CardTitle>Browse</CardTitle>
                        <div className="mt-1 text-xs text-muted-foreground">Source context mappings from operation field paths to semantic meaning</div>
                      </div>
                      <Button size="sm" variant="outline" onClick={openCreateMapping} disabled={!canEditCurrentView}>
                        <Plus className="h-4 w-4" />
                        New mapping
                      </Button>
                    </div>
                    ...
                    {loading ? (
                      ...
                      <>
                          ...
                          {paginatedMappings.map((mapping) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0 flex-1">
                                    <div className="truncate text-[13px] font-medium">{display.field_path}</div>
                                    <div className="mt-1 truncate text-[11px] text-muted-foreground">
                                      {display.operation_id} · {display.semantic_type_id}
                                    </div>
                                  </div>
                                  <div className="flex shrink-0 flex-col items-end gap-1">
                                    <Badge variant="info">{display.mapping_type || "exact"}</Badge>
                                    {mapping.draft_snapshot ? <Badge variant="warning">Draft</Badge> : null}
                                  </div>
                                </div>
                              </button>
                            );
                        ...
                        <PaginationBar
                          ...
                          totalPages={totalMappingsPages}
                          onPageChange={setCurrentMappingsPage}
                        />
                      </>
                    ) : (
                      <div className="p-4">
                ...
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div>
                      <CardTitle>{labels.sections.inspector}</CardTitle>
                      <div className="mt-1 text-xs text-muted-foreground">Selected mapping context, canonical link, transform, and provenance</div>
                    </div>
                  </CardHeader>
                  <CardContent className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                    {mappingEditorMode === "create" || selectedMapping ? (
                      <>
                        {mappingEditorMode === "detail" && selectedMapping ? (
                          (() => {
                            const display = selectedMapping.draft_snapshot || selectedMapping;
                            return (
                              <div className="space-y-3">
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <div className="text-lg font-semibold">{display.field_path}</div>
                                    <div className="mt-1 text-xs text-muted-foreground">
                                      {display.operation_id} · {display.mapping_type || "exact"}
                                    </div>
                                  </div>
                                  <div className="flex flex-wrap gap-2">
                                    <Badge variant={selectedMapping.status === "approved" ? "success" : "warning"}>
                                      {selectedMapping.status === "approved" ? labels.status.approved : labels.status.draft}
                                    </Badge>
                                    {selectedMapping.draft_snapshot ? <Badge variant="warning">Draft changes</Badge> : null}
                                  </div>
                                </div>
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <MetaPill label="Mapping ID" value={selectedMapping.id} icon={GitBranch} />
                                  <MetaPill label="Updated" value={formatDate(selectedMapping.updated_at, language)} />
                                  <MetaPill label="Source" value={display.source_id || "-"} />
                                  <MetaPill label="Operation" value={display.operation_id} />
                                </div>
                              </div>
                            );
                          })()
                          ...
                          <div className="space-y-1">
                            <div className="text-lg font-semibold">Create mapping</div>
                            <div className="text-xs text-muted-foreground">
                              Bind a source context to a semantic type and canonical attribute.
                            </div>
                          </div>
                        )}

                        <form className="space-y-4" onSubmit={submitMapping}>
                          <div className="grid gap-4 md:grid-cols-2">
                            <label className="space-y-2 text-sm">
                              <span>Source</span>
                              <select
                                ...
                                onChange={(event) => setMappingForm((current) => ({ ...current, sourceId: event.target.value }))}
                              >
                            ...
                            <label className="space-y-2 text-sm">
                              ...
                              <select
                                className="h-10 w-full rounded-lg border border-border/80 bg-muted/50 px-3 text-sm outline-none focus:border-primary/70 focus:ring-2 focus:ring-primary/20"
                                value={mappingForm.operationId}
                                onChange={(event) => setMappingForm((current) => ({ ...current, operationId: event.target.value }))}
                              >
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            <label className="space-y-2 text-sm">
                              ...
                              <Input
                                value={mappingForm.fieldPath}
                                onChange={(event) => setMappingForm((current) => ({ ...current, fieldPath: event.target.value }))}
                              />
                            </label>
                            <label className="space-y-2 text-sm">
                              ...
                              <select
                                className="h-10 w-full rounded-lg border border-border/80 bg-muted/50 px-3 text-sm outline-none focus:border-primary/70 focus:ring-2 focus:ring-primary/20"
                                value={mappingForm.semanticTypeId}
                                onChange={(event) => setMappingForm((current) => ({ ...current, semanticTypeId: event.target.value }))}
                              >
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            <label className="space-y-2 text-sm">
                              ...
                              <select
                                className="h-10 w-full rounded-lg border border-border/80 bg-muted/50 px-3 text-sm outline-none focus:border-primary/70 focus:ring-2 focus:ring-primary/20"
                                value={mappingForm.canonicalAttributeId}
                                onChange={(event) =>
                                  setMappingForm((current) => ({ ...current, canonicalAttributeId: event.target.value }))
                                }
                              >
                                <option value="">Optional</option>
                                {canonicalAttributeOptions.map((attribute) => (
                                  <option key={attribute.id} value={attribute.id}>
                                    {(attribute.draft_snapshot || attribute).name}
                                  </option>
                                ))}
                              </select>
                            </label>
                            <label className="space-y-2 text-sm">
                              ...
                              <select
                                ...
                                onChange={(event) => setMappingForm((current) => ({ ...current, mappingType: event.target.value }))}
                              >
                                {["exact", "transform", "composite", "enum", "reference"].map((option) => (
                          ...
                          <div className="grid gap-4 md:grid-cols-3">
                            <label className="space-y-2 text-sm">
                              <span>Namespace</span>
                              <Input
                                value={mappingForm.namespace}
                                onChange={(event) => setMappingForm((current) => ({ ...current, namespace: event.target.value }))}
                              />
                            </label>
                            <label className="space-y-2 text-sm">
                              <span>Version</span>
                              <Input
                                ...
                                onChange={(event) => setMappingForm((current) => ({ ...current, version: event.target.value }))}
                              />
                            </label>
                            <label className="space-y-2 text-sm">
                              ...
                              <Input
                                ...
                                onChange={(event) => setMappingForm((current) => ({ ...current, confidence: event.target.value }))}
                              />
                            </label>
                          </div>
                          <label className="space-y-2 text-sm">
                            <span>Notes</span>
                            <textarea
                              className="min-h-[92px] w-full rounded-lg border border-border/80 bg-muted/50 px-3 py-2 text-sm outline-none focus:border-primary/70 focus:ring-2 focus:ring-primary/20"
                              value={mappingForm.notes}
                              onChange={(event) => setMappingForm((current) => ({ ...current, notes: event.target.value }))}
                            />
                          </label>
                          <div className="grid gap-4 md:grid-cols-2">
                            <label className="space-y-2 text-sm">
                              ...
                              <textarea
                                className="min-h-[160px] w-full rounded-lg border border-border/80 bg-muted/50 px-3 py-2 font-mono text-xs outline-none focus:border-primary/70 focus:ring-2 focus:ring-primary/20"
                                value={mappingForm.transformSpec}
                                onChange={(event) =>
                                  setMappingForm((current) => ({ ...current, transformSpec: event.target.value }))
                                }
                              />
                            </label>
                            <label className="space-y-2 text-sm">
                              <span>Enum Mapping JSON</span>
                              <textarea
                                className="min-h-[160px] w-full rounded-lg border border-border/80 bg-muted/50 px-3 py-2 font-mono text-xs outline-none focus:border-primary/70 focus:ring-2 focus:ring-primary/20"
                                value={mappingForm.enumMapping}
                                onChange={(event) =>
                                  setMappingForm((current) => ({ ...current, enumMapping: event.target.value }))
                                }
                              />
                            </label>
                          </div>
                          <div className="flex flex-wrap gap-2 border-t border-border pt-4">
                            <Button type="submit" disabled={!canEditCurrentView || submitting === "mapping" || submitting === "create-mapping"}>
                              {submitting === "mapping" || submitting === "create-mapping" ? (
                                <LoaderCircle className="h-4 w-4 animate-spin" />
                              ) : (
                                <PencilLine className="h-4 w-4" />
                              )}
                              {mappingEditorMode === "create" ? "Create mapping" : labels.actions.save}
                            </Button>
                            {mappingEditorMode === "detail" && selectedMapping ? (
                              <Button
                                type="button"
                                ...
                                disabled={!canEditCurrentView || submitting === "delete-mapping"}
                              >
                                {submitting === "delete-mapping" ? (
                                  <LoaderCircle className="h-4 w-4 animate-spin" />
                                ) : (
                                  <Trash2 className="h-4 w-4" />
                                )}
                      ...
                      <EmptyState icon={Eye} label="Select a mapping to inspect." />
                    )}
                  </CardContent>
                </Card>
              </section>
              ...
              <section className="grid min-h-0 gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div>
                      <CardTitle>Browse</CardTitle>
                      <div className="mt-1 text-xs text-muted-foreground">Planner-facing capabilities and their semantic coverage</div>
                    </div>
                  </CardHeader>
                  <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                    {loading ? (
                      ...
                      <>
                              ...
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0 flex-1">
                                    <div className="truncate text-[13px] font-medium">{display.name}</div>
                                    <div className="mt-1 truncate text-[11px] text-muted-foreground">
                                      {display.capability_key} · in {display.input_semantic_types?.length || 0} · out {display.output_semantic_types?.length || 0}
                                    </div>
                                  </div>
                        ...
                        <PaginationBar
                          currentPage={currentCapabilitiesPage}
                          totalPages={totalCapabilitiesPages}
                          onPageChange={setCurrentCapabilitiesPage}
                        />
                      </>
                    ) : (
                      <div className="p-4">
                ...
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div>
                      <CardTitle>{labels.sections.inspector}</CardTitle>
                      <div className="mt-1 text-xs text-muted-foreground">Capability intent, semantic inputs/outputs, and governance status</div>
                    </div>
                  </CardHeader>
                  <CardContent className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                    {!selectedCapability ? (
                      <EmptyState icon={Eye} label="Select a capability to inspect." />
                    ) : (
                      (() => {
                        ...
                        return (
                          <>
                            <div className="space-y-3">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  ...
                                  <div className="mt-1 text-xs text-muted-foreground">
                                    {display.capability_key} · {display.namespace || "public"}
                                  </div>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                  <Badge variant={selectedCapability.status === "approved" ? "success" : "warning"}>
                                    {selectedCapability.status === "approved" ? labels.status.approved : labels.status.draft}
                                  </Badge>
                                  {selectedCapability.draft_snapshot ? <Badge variant="warning">Draft changes</Badge> : null}
                                </div>
                              </div>
                              <div className="grid gap-2 sm:grid-cols-2">
                                <MetaPill label="Capability ID" value={selectedCapability.id} icon={Sparkles} />
                                <MetaPill label="Updated" value={formatDate(selectedCapability.updated_at, language)} />
                                <MetaPill label="Version" value={display.version || "1.0.0"} />
                                <MetaPill label="Lifecycle" value={display.lifecycle || "draft"} />
                              </div>
                            </div>
                            ...
                            <InspectorList label="Inputs" values={display.input_semantic_types || []} emptyLabel="No semantic inputs" />
                            <InspectorList label="Outputs" values={display.output_semantic_types || []} emptyLabel="No semantic outputs" />
                            <JsonBlock title="Intent Spec" value={display.intent_spec || {}} />
                          </>
                        );
                      })()
                    )}
            ...
            {activeModule === "operations" ? (
              <section className="flex flex-col gap-4">
                    ...
                    <div className="inline-flex rounded-lg border border-border bg-muted/20 p-1">
                      {([
                        ...
                        <button
                          key={tab.id}
                          type="button"
                          onClick={() => setActiveExecutionTab(tab.id)}
                          className={`rounded-md px-3 py-1.5 text-sm transition ${
                            activeExecutionTab === tab.id
                              ? "bg-primary text-primary-foreground shadow-sm"
                              : "text-muted-foreground hover:bg-accent hover:text-foreground"
                          }`}
                        >
                ...
                {activeExecutionTab === "sources" ? (
                <div className="grid min-h-0 gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
                  <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                    <CardHeader className="relative border-b border-border p-4 pr-28">
                      <div>
                        <CardTitle>Browse</CardTitle>
                        <div className="mt-1 text-xs text-muted-foreground">Sources available in the current operations registry</div>
                      </div>
                      <Button
                        size="sm"
                        variant="outline"
                  ...
                  <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                    <CardHeader className="border-b border-border p-4">
                      <div>
                        <CardTitle>{labels.sections.inspector}</CardTitle>
                        <div className="mt-1 text-xs text-muted-foreground">Selected source details and edit actions</div>
                      </div>
                    </CardHeader>
                    <CardContent className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                ...
                ) : activeExecutionTab === "assets" ? (
                  <div className="grid min-h-0 gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
                    <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                      <CardHeader className="border-b border-border p-4">
                        <div>
                          <CardTitle>Browse</CardTitle>
                          <div className="mt-1 text-xs text-muted-foreground">Physical assets grouped under each execution source</div>
                        </div>
                      </CardHeader>
                      <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                        {loading ? (
                        ...
                        ) : paginatedExecutionAssets.length ? (
                          <>
                            <div className="flex-1 divide-y divide-border/70 overflow-auto">
                              {paginatedExecutionAssets.map((asset) => {
                                const isSelected = asset.id === selectedExecutionAssetId;
                                return (
                                  <button
                                    key={asset.id}
                                    type="button"
                                    onClick={() => selectExecutionAsset(asset)}
                                    className={`block w-full px-4 py-3 text-left transition hover:bg-muted/20 ${
                                      isSelected ? "bg-primary/5" : ""
                                    }`}
                                  >
                                    <div className="flex items-start justify-between gap-3">
                                      <div className="min-w-0">
                                        ...
                                        <div className="mt-1 truncate text-xs text-muted-foreground">
                                          {asset.asset_type} · {asset.source_name || asset.source_id}
                                        </div>
                                      </div>
                                      <Badge variant="info">{asset.status || "draft"}</Badge>
                                    </div>
                                  </button>
                                );
                              })}
                            ...
                            <PaginationBar
                              currentPage={currentAssetsPage}
                              totalPages={totalAssetsPages}
                              onPageChange={setCurrentAssetsPage}
                            />
                          </>
                        ) : (
                          <div className="p-4">
                            <EmptyState icon={Database} label="No execution assets yet." />
                          </div>
                        )}
                      </CardContent>
                    </Card>
                    <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                      <CardHeader className="border-b border-border p-4">
                        <div>
                          <CardTitle>{labels.sections.inspector}</CardTitle>
                          <div className="mt-1 text-xs text-muted-foreground">Selected asset context and source binding</div>
                        </div>
                      </CardHeader>
                      <CardContent className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                        {!selectedExecutionAsset ? (
                          <EmptyState icon={Eye} label="Select an asset to inspect." />
                        ) : (
                          <>
                            <div className="space-y-3">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <div className="text-lg font-semibold">{selectedExecutionAsset.name}</div>
                                  <div className="mt-1 text-xs text-muted-foreground">
                                    {selectedExecutionAsset.asset_type} · {selectedExecutionAsset.source_name}
                                  </div>
                                </div>
                                <Badge variant={selectedExecutionAsset.status === "approved" ? "success" : "warning"}>
                                  {selectedExecutionAsset.status || "draft"}
                                </Badge>
                              </div>
                              <div className="grid gap-2 sm:grid-cols-2">
                                <MetaPill label="Asset ID" value={selectedExecutionAsset.id} icon={Database} />
                                <MetaPill label="Updated" value={formatDate(selectedExecutionAsset.updated_at, language)} />
                                <MetaPill label="Source" value={selectedExecutionAsset.source_name || selectedExecutionAsset.source_id} />
                                <MetaPill label="Locator" value={selectedExecutionAsset.locator || "-"} />
                              </div>
                            </div>
                            {selectedExecutionAsset.description ? (
                              <div className="rounded-xl border border-border/70 bg-muted/15 p-4 text-sm leading-6">
                                {selectedExecutionAsset.description}
                              </div>
                            ) : null}
                            <JsonBlock title="Metadata" value={selectedExecutionAsset.metadata || {}} />
                          </>
                        )}
                      </CardContent>
                    </Card>
                  ...
                  <div className="grid min-h-0 gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
                    <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                      <CardHeader className="border-b border-border p-4">
                        <div>
                          <CardTitle>Browse</CardTitle>
                          <div className="mt-1 text-xs text-muted-foreground">Logical operations, access paths, and field contracts</div>
                        </div>
                      </CardHeader>
                      <CardContent className="flex min-h-0 flex-1 flex-col p-0">
                        {loading ? (
                        ...
                        ) : paginatedExecutionOperations.length ? (
                          <>
                            <div className="flex-1 divide-y divide-border/70 overflow-auto">
                              {paginatedExecutionOperations.map((operation) => {
                                const isSelected = operation.id === selectedExecutionOperationId;
                                return (
                                  <button
                                    key={operation.id}
                                    type="button"
                                    onClick={() => selectExecutionOperation(operation)}
                                    className={`block w-full px-4 py-3 text-left transition hover:bg-muted/20 ${
                                      isSelected ? "bg-primary/5" : ""
                                    }`}
                                  >
                                    <div className="flex items-start justify-between gap-3">
                                      <div className="min-w-0">
                                        <div className="truncate font-medium">{operation.name}</div>
                                        <div className="mt-1 truncate text-xs text-muted-foreground">
                                          {operation.operation_key} · {operation.source_name || operation.asset_name}
                                        </div>
                                      </div>
                                      <Badge variant="info">{operation.http_method || operation.access_type || "op"}</Badge>
                                    </div>
                                  </button>
                                );
                              })}
                          ...
                          <div className="p-4">
                            <EmptyState icon={GitBranch} label="No execution operations yet." />
                          </div>
                        )}
                      </CardContent>
                    </Card>
                    <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                      <CardHeader className="border-b border-border p-4">
                        <div>
                          <CardTitle>{labels.sections.inspector}</CardTitle>
                          <div className="mt-1 text-xs text-muted-foreground">Operation contract, access path, and extracted field paths</div>
                        </div>
                      </CardHeader>
                      <CardContent className="min-h-0 flex-1 space-y-4 overflow-auto p-4">
                        {!selectedExecutionOperation ? (
                          <EmptyState icon={Eye} label="Select an operation to inspect." />
                        ) : (
                          <>
                            <div className="space-y-3">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <div className="text-lg font-semibold">{selectedExecutionOperation.name}</div>
                                  <div className="mt-1 text-xs text-muted-foreground">
                                    {selectedExecutionOperation.operation_key} · {selectedExecutionOperation.source_name}
                                  </div>
                                </div>
                                <Badge variant={selectedExecutionOperation.status === "approved" ? "success" : "warning"}>
                                  {selectedExecutionOperation.status || "draft"}
                                </Badge>
                              </div>
                              <div className="grid gap-2 sm:grid-cols-2">
                                <MetaPill label="Operation ID" value={selectedExecutionOperation.id} icon={GitBranch} />
                                <MetaPill label="Updated" value={formatDate(selectedExecutionOperation.updated_at, language)} />
                                <MetaPill label="Asset" value={selectedExecutionOperation.asset_name || "-"} />
                                <MetaPill
                                  label="Access Path"
                                  value={`${selectedExecutionOperation.http_method || selectedExecutionOperation.access_type || ""} ${selectedExecutionOperation.access_path_locator || ""}`.trim() || "-"}
                                />
                              </div>
                            </div>
                            {selectedExecutionOperation.description ? (
                              <div className="rounded-xl border border-border/70 bg-muted/15 p-4 text-sm leading-6">
                                {selectedExecutionOperation.description}
                              </div>
                            ) : null}
                            <OperationFieldSection title="Input Fields" scope="input" fields={operationFieldsForSelection} />
                            <OperationFieldSection title="Output Fields" scope="output" fields={operationFieldsForSelection} />
                            <OperationFieldSection title="Control Fields" scope="control" fields={operationFieldsForSelection} />
                            <JsonBlock title="Input Spec" value={selectedExecutionOperation.input_spec || []} />
                            <JsonBlock title="Output Spec" value={selectedExecutionOperation.output_spec || []} />
                            <JsonBlock title="Auth Spec" value={selectedExecutionOperation.auth_spec || {}} />
                            <JsonBlock title="Contract Spec" value={selectedExecutionOperation.contract_spec || {}} />
                          </>
                        )}
                      </CardContent>
                    </Card>
            ...
            {activeModule === "governance" ? (
              <section className="grid min-h-0 gap-4 xl:grid-cols-[minmax(0,1.12fr)_minmax(360px,0.88fr)]">
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <CardTitle>Browse</CardTitle>
                        <div className="mt-1 text-xs text-muted-foreground">Governance proposals in review order</div>
                      </div>
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0">
                                    <div className="truncate text-[13px] font-medium">{proposal.title}</div>
                                    <div className="mt-1 text-[11px] text-muted-foreground">
                                      {proposal.entity_type} · {proposal.change_type} · {formatDate(proposal.created_at, language)}
                                    </div>
                                    <div className="mt-1 truncate text-[11px] text-foreground/75">
                ...
                <Card className="workspace-panel flex h-[720px] min-h-0 flex-col overflow-hidden rounded-2xl shadow-none">
                  <CardHeader className="border-b border-border p-4">
                    <div>
                      <CardTitle>{labels.sections.inspector}</CardTitle>
                      <div className="mt-1 text-xs text-muted-foreground">Selected proposal diff and review actions</div>
                    </div>
                  </CardHeader>
...
function JsonBlock({ title, value }: { title: string; value: Record<string, unknown> | unknown[] }) {
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
      ...
      {isEmpty ? (
        ...
        <pre className="overflow-x-auto whitespace-pre-wrap break-all text-xs leading-6 text-foreground/85">
...
function OperationFieldSection({
  title,
  scope,
  fields
}: {
      ...
      {scopedFields.length ? (
        <div className="space-y-2">
          {scopedFields.map((field) => (
            <div key={field.id} className="rounded-lg border border-border/70 bg-background px-3 py-2">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium">{field.field_path || field.raw_name}</div>
                  <div className="mt-1 truncate text-[11px] text-muted-foreground">
                    {field.data_type || "string"} · {field.is_required ? "required" : "optional"}
                  </div>
                </div>
                <Badge variant="default">{field.raw_name}</Badge>
              </div>
              {field.description ? <div className="mt-2 text-xs text-muted-foreground">{field.description}</div> : null}
            </div>
          ))}
        </div>
      ) : (
        <div className="text-sm text-muted-foreground">No fields registered.</div>
      )}
    </div>
  );
}
...
function RelationshipDirectionSection({
  title,
  relationships,
  onSelectRelationship
}: {
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-muted/15 p-4">
      <div className="mb-3 text-xs uppercase tracking-[0.14em] text-muted-foreground">{title}</div>
      {relationships.length ? (
        <div className="space-y-2">
          {relationships.map((relationship) => (
            <button
...
function summarizeProposal(proposal: Proposal) {
  const payload = (proposal.payload || {}) as Record<string, unknown>;
  const draft = (payload.draft_snapshot || payload.entity || payload.relationship || payload.source || payload) as
    | Record<string, unknown>
    | undefined;
  const approved = payload.approved_snapshot as Record<string, unknown> | undefined;

  if (proposal.change_type === "delete") {
    const name =
      String(approved?.name || approved?.relation_type || draft?.name || draft?.relation_type || proposal.entity_id || "").trim();
    return name ? `Delete ${name}` : "Delete pending item";
  }

  if (proposal.change_type === "create") {
    const name =
      String(draft?.name || draft?.relation_type || approved?.name || proposal.entity_id || "").trim();
    return name ? `Create ${name}` : "Create pending item";
  }

  const fromName = String(approved?.name || approved?.relation_type || "").trim();
  const toName = String(draft?.name || draft?.relation_type || "").trim();
  if (fromName && toName && fromName !== toName) {
    return `${fromName} → ${toName}`;
  }
  const summary =
    String(draft?.description || approved?.description || draft?.name || approved?.name || proposal.entity_id || "").trim();
  return summary || "Update pending item";
}
