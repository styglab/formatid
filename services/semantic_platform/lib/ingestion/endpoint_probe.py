...
def _probe_endpoint_candidate(section: dict[str, Any], base_url: str, state: SourceGraphState) -> dict[str, Any]:
    ...
    attempts = max(1, _candidate_probe_retry_count() + 1)
    last_timeout: TimeoutError | URLError | None = None
    ...
    return _endpoint_candidate_inconclusive(
        section,
        "timeout",
        str(last_timeout or "request timed out"),
        url,
        raw_arguments,
        started,
        raw_body,
...
def _candidate_probe_status(body: bytes, content_type: str) -> tuple[str, str, str]:
    try:
        payload, response_format = _parse_response_payload(body, content_type)
    except (json.JSONDecodeError, ValueError) as exc:
        return "unparseable_response", f"unparseable response body: {exc}", "failed"
    if payload is None:
        return "empty", "empty response body", "failed"
    return "http_success", f"probe returned parseable {response_format} response", "passed"
...
def _candidate_service_key(state: SourceGraphState) -> str | None:
    secret_value = _candidate_service_key_from_source_secret(state)
    if secret_value:
        return secret_value
...
def _candidate_service_key_parameter(state: SourceGraphState) -> str:
    source_parameter = _candidate_service_key_parameter_from_source(state)
    if source_parameter:
...
def _candidate_service_key_from_source_secret(state: SourceGraphState) -> str | None:
    metadata = state.get("source_document", {}).get("metadata")
    if not isinstance(metadata, dict):
        return None
    refs = metadata.get("auth_secret_refs")
    if not isinstance(refs, list):
        return None
    repo = SemanticCatalogRepository()
    for ref in refs:
        value = repo.secret_value(str(ref))
        if value:
            return value
    return None


def _candidate_service_key_parameter_from_source(state: SourceGraphState) -> str | None:
    metadata = state.get("source_document", {}).get("metadata")
    if not isinstance(metadata, dict):
        return None
    names = metadata.get("auth_parameter_names")
...
def _candidate_probe_timeout_seconds() -> float:
    try:
        return float(os.getenv("SEMANTIC_PLATFORM_CANDIDATE_PROBE_TIMEOUT_SECONDS", "15"))
    except ValueError:
        return 15.0


def _candidate_probe_retry_count() -> int:
    try:
        return max(0, int(os.getenv("SEMANTIC_PLATFORM_CANDIDATE_PROBE_RETRIES", "1")))
    except ValueError:
        return 1
...
def _verify_http_variant(
    ...
    semantic_arguments = {
        **(variant.get("fixed_semantic_arguments") if isinstance(variant.get("fixed_semantic_arguments"), dict) else {}),
        **sample_semantic_arguments,
    }
    request_parts = _raw_arguments_by_contract_section(semantic_arguments, contract)
    ...
    full_url = f"{url}?{parse.urlencode(raw_arguments, doseq=True)}"
    attempts = max(1, _verification_retry_count() + 1)
    last_timeout: TimeoutError | URLError | None = None
    for attempt in range(1, attempts + 1):
        started = datetime.now(timezone.utc)
        try:
            ...
            if method == "POST":
                ...
                http_request = full_url
            with request.urlopen(http_request, timeout=_verification_timeout_seconds()) as response:
                body = response.read()
                content_type = response.headers.get("content-type", "")
            status, provider_message, result_status, normalized_sample = _provider_status(body, content_type, contract)
            return {
                "variant_id": variant.get("variant_id"),
                "operation_id": variant.get("operation_id"),
                "capability_id": variant.get("capability_id") or variant.get("capability"),
...
def _raw_arguments_by_contract_section(
    ...
    for section in ("query", "body", "path", "header"):
        ...
        if not isinstance(fields, dict):
            continue
        _collect_contract_fields(raw_arguments, section, fields, semantic_arguments)
    flat_fields = request_contract.get("fields")
    if isinstance(flat_fields, dict):
        for raw_name, field in flat_fields.items():
            if not isinstance(field, dict):
                continue
            location = str(field.get("location") or "query").lower()
            section = location if location in raw_arguments else "query"
            _collect_contract_fields(raw_arguments, section, {raw_name: field}, semantic_arguments)
    defaults = request_contract.get("defaults")
    if isinstance(defaults, dict):
        for raw_name, value in defaults.items():
            if raw_name not in raw_arguments["query"]:
                raw_arguments["query"][str(raw_name)] = value
    return {
    ...
    for raw_name, field in fields.items():
        ...
        if semantic_type in semantic_arguments:
            raw_arguments[section][str(raw_name)] = _contract_value(
                ...
                str(field.get("format") or ""),
            )
        elif raw_name in semantic_arguments:
            raw_arguments[section][str(raw_name)] = semantic_arguments[raw_name]
        elif "default" in field:
            raw_arguments[section][str(raw_name)] = field.get("default")


def _contract_value(value: Any, transform: str, value_format: str = "") -> Any:
    if isinstance(value, dict):
        if transform == "date_start":
            return _format_contract_value(value.get("from") or value.get("start"), value_format)
        if transform == "date_end":
            return _format_contract_value(value.get("to") or value.get("end"), value_format)
    return _format_contract_value(value, value_format)


def _format_contract_value(value: Any, value_format: str) -> Any:
    if value is None:
        return None
    if value_format in {"yyyyMMddHHmm", "YYYYMMDDHHmm"}:
        return str(value).replace("-", "").replace(":", "").replace("T", "").replace(" ", "")[:12]
    if value_format in {"yyyyMMdd", "YYYYMMDD"}:
        return str(value).replace("-", "").replace(":", "").replace("T", "").replace(" ", "")[:8]
    return value
...
def _api_key(auth: dict[str, Any]) -> str | None:
    ...
    for name in [str(value) for value in names]:
        for secret_id in _secret_id_candidates(name):
            if repo is None:
                repo = SemanticCatalogRepository()
            try:
                secret_value = repo.secret_value(secret_id)
            except Exception:
                secret_value = None
            if secret_value:
                return secret_value
...
def _secret_id_candidates(name: str) -> list[str]:
    key = name.strip()
    candidates = [key]
    if key and not key.startswith("secret."):
        candidates.append(f"secret.{key.lower()}")
...
def _provider_status(body: bytes, content_type: str, contract: dict[str, Any]) -> tuple[str, str, str, dict[str, Any]]:
    payload, response_format = _parse_response_payload(body, content_type)
    if payload is None:
        return "unknown", "empty response body", "failed", {}
    status, message, result_status = _provider_status_from_contract(payload, contract)
    return status or response_format, message, result_status, _sample_payload(payload)
...
def _provider_status_from_contract(payload: Any, contract: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if error:
        ...
        if "equals" in error and _matches_rule(code, error.get("equals")):
            return str(code), str(message or "declared error condition matched"), "failed"
        if "not_equals" in error and not _matches_rule(code, error.get("not_equals")):
            return str(code), str(message or "declared error condition matched"), "failed"
        if "in" in error and _matches_any(code, error.get("in")):
            return str(code), str(message or "declared error condition matched"), "failed"
        if "not_in" in error and not _matches_any(code, error.get("not_in")):
            return str(code), str(message or "declared error condition matched"), "failed"
    if success:
        code = _first_path_value(payload, str(success.get("path") or ""))
        message = _first_path_value(payload, str(success.get("message_path") or ""))
        if code is None:
            return "unknown", str(message or "declared success path not found"), "failed"
        if "equals" in success:
            passed = _matches_rule(code, success.get("equals"))
            return str(code), str(message or ""), "passed" if passed else "failed"
        if "in" in success:
            passed = _matches_any(code, success.get("in"))
            return str(code), str(message or ""), "passed" if passed else "failed"
        if "exists" in success:
            passed = bool(code) if success.get("exists") else code in (None, "")
            return str(code), str(message or ""), "passed" if passed else "failed"
    return "OK", "", "passed"


def _matches_rule(value: Any, expected: Any) -> bool:
    return str(value) == str(expected)


def _matches_any(value: Any, expected_values: Any) -> bool:
    if not isinstance(expected_values, list):
        return False
    return str(value) in {str(item) for item in expected_values}


def _parse_response_payload(body: bytes, content_type: str) -> tuple[Any, str]:
    ...
    if stripped.startswith("<"):
        try:
            ...
            raise ValueError(f"unparseable response body: {exc}") from exc
        return {root.tag.split("}", 1)[-1]: _xml_element_to_value(root)}, "xml"
...
def _sample_payload(payload: Any) -> dict[str, Any]:
    if isinstance(payload, dict):
        return payload
    if isinstance(payload, list):
        return {"items": payload[:3]}
    return {"value": payload}


def _verification_timeout_seconds() -> float:
    try:
        return float(os.getenv("SEMANTIC_PLATFORM_VERIFICATION_TIMEOUT_SECONDS", "15"))
    except ValueError:
        return 15.0


def _verification_retry_count() -> int:
    try:
        return max(0, int(os.getenv("SEMANTIC_PLATFORM_VERIFICATION_RETRIES", "1")))
    except ValueError:
        return 1
