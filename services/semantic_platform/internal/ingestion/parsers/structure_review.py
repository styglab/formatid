...
def _discover_document_fields(loaded: LoadedSource, operations: list[dict[str, Any]]) -> list[StructureOperationDraft]:
    if not operations:
        return []
    lines = [line.strip() for line in loaded.content_text.splitlines() if line.strip()]
    candidates = [line for line in lines[:40] if ":" in line and len(line.split(":")[0].strip()) < 64]
    if not candidates:
        return []
    ...
    for candidate in candidates:
        raw_name = candidate.split(":", 1)[0].strip().replace(" ", "_")
        ...
        fields.append(
            DiscoveredField(
                scope="output",
                raw_name=raw_name,
                field_path=f"document.{raw_name}",
                data_type="string",
                is_required=False,
                description=candidate[:200],
                evidence=[{"kind": "document_line", "text": candidate[:200]}],
            )
        )
    return [StructureOperationDraft(operation_key=operation_key, fields=fields)]
