...
def _ensure_collection(collection: str, vector_size: int) -> None:
    response = _request("GET", f"{_qdrant_url()}/collections/{collection}", allow_404=True)
...
def _embedding_url() -> str:
    return os.getenv("SEMANTIC_PLATFORM_EMBEDDING_URL", "http://embedding-service:8000/embeddings").rstrip("/")


def _qdrant_url() -> str:
    return os.getenv("SEMANTIC_PLATFORM_QDRANT_URL", "http://qdrant:6333").rstrip("/")


def _collection_name() -> str:
    return os.getenv("SEMANTIC_PLATFORM_SEMANTIC_TYPE_COLLECTION", "semantic_platform_semantic_types")
