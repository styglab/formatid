...
`services/worker`는 앱 서비스들이 공유하는 generic worker runtime입니다.

운영 조회 API는 별도 [services/api](../api) 에서 제공합니다. health 조회는 worker queue task가 아니라 Redis heartbeat를 직접 읽는 방식입니다.
API 이미지와 의존성 정의는 [infra/images/api](../../infra/images/api) 에 둡니다.
...
worker service 메타데이터 원본은 `apps/*/manifests/*worker.json` 과 `services/*/manifests/*worker.json` 입니다.
health/ops에서 사용하는 기본 queue 목록과 expected worker 수는 [service_catalog.py](../catalog/service_catalog.py) 가 이 manifest를 읽어서 제공합니다.

이미지/의존성은 `infra/images/workers` 아래 worker family별로 분리합니다.

- [Dockerfile](../../infra/images/workers/ingest_api/Dockerfile): `ingest:api`
- [Dockerfile](../../infra/images/workers/ingest_file/Dockerfile): `ingest:file`
...
- [worker.common.env](../../infra/env/worker.common.env): 공통 설정
- [worker.ingest_api.env](../../infra/env/workers/worker.ingest_api.env): `ingest:api`
- [worker.ingest_file.env](../../infra/env/workers/worker.ingest_file.env): `ingest:file`

외부 producer에서 task 넣기:
```bash
cd /path/to/project-root
python3 scripts/ops.py enqueue ingest:api ingest.api.fetch --payload '{"request":{"method":"GET","url":"http://api:8000/health/live"},"target":{"type":"postgres","schema":"public","table":"generic_api_ingest","create_table":true}}'
```
...
즉 producer는 대상 큐 이름(`ingest:api`, `ingest:file`)에 이 JSON을 `RPUSH` 하면 되고, worker는 자신이 맡은 큐를 `BLPOP`으로 꺼내 실행합니다.
...
worker 서비스 원본은 `apps/*/manifests/*worker.json` 과 `services/*/manifests/*worker.json` 입니다.
platform 서비스 원본은 [infra/platform_services](../../infra/platform_services) 아래 manifest입니다.
...
1. `apps/<app>/tasks/...` 아래에 task 구현 추가
2. `apps/<app>/manifests/tasks.json` 에 task manifest 추가
3. `apps/<app>/manifests/<service>_worker.json` 에 worker service manifest 추가
4. 필요 시 `infra/images` 와 `infra/env` 에 service 전용 정의 추가
5. compose 재생성
...
또한 `api`, app service, 각 worker는 모두 `restart: unless-stopped` 정책으로 자동 재기동됩니다.
