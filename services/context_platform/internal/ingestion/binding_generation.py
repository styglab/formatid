    ...
    *,
    ...
    return {
        ...
        "instructions": [
            ...
            "Separate value fields from context fields. Put measured/status/identifier output values in field_bindings and put subject, subject_identifier, currency, fiscal_year, statement_type, source, and observed_date fields in context_bindings.",
            "If response outputs are repeated records, arrays, items, or data[] rows and a source field identifies the row subject, create a context_binding with context_key subject_identifier rather than exposing that identifier as a primary output.",
            "If the RepresentationSchema is a code-valued schema such as schema.*.code, keep value_domain_key or enum_mapping when supplied by meaning_resolution. Do not downgrade governed codes to plain strings.",
            ...
            "context_bindings": [
                {
                    ...
                    "context_key": "subject_identifier|subject|currency|fiscal_year|statement_type|source|observed_date",
                    "direction": "output",
