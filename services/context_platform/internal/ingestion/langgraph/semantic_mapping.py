...
from services.context_platform.internal.ingestion.llm.semantic_mapping import (
...
def generate_semantic_mapping_drafts(
    *,
    ...
    canonical_index = _build_canonical_class_slot_usage_index(canonical_class_slot_usages)
    ...
    for field in fields:
        ...
        for name in candidate_names:
            ...
            canonical_class_slot_id = str((canonical_match or {}).get("id") or "") or None
        ...
        if semantic_type_id:
            ...
            if semantic_status == "approved" and canonical_status == "approved" and canonical_class_slot_id:
                resolution_basis = "approved"
                dependency_status = "ready"
                review_impact = []
            elif canonical_class_slot_id:
...
def _build_canonical_class_slot_usage_index(canonical_class_slot_usages: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    index: dict[str, dict[str, Any]] = {}
    for attribute in canonical_class_slot_usages:
...
def _serialize_suggestion(suggestion: SemanticMappingSuggestion) -> dict[str, Any]:
    return {
        ...
        "canonical_class_slot_id": suggestion.canonical_class_slot_id,
