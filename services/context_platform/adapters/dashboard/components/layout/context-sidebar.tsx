...
import { contextNavSections } from "@/lib/context/navigation";
...
export function ContextSidebar({ collapsed, onToggle }: ContextSidebarProps) {
  const pathname = usePathname();
  const normalizedPathname =
    pathname === "/context-platform"
      ? "/"
      : pathname.startsWith("/context-platform/")
        ? pathname.slice("/context-platform".length)
        : pathname;
  ...
  return (
    ...
    >
      <div className={`mb-4 flex px-2 ${collapsed ? "justify-center" : "items-center justify-between"}`}>
        {!collapsed ? (
          <div>
            <div className="text-sm font-semibold">Context Platform</div>
      ...
      <div className="space-y-4">
        {contextNavSections.map((section) => (
