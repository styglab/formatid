...
from apps.g2b_pipeline.app.repositories.common import acquire_write_lock, load_raw_rows
from apps.g2b_pipeline.app.steps.bid_notices import CATEGORY_BY_LABEL
from apps.g2b_pipeline.app.steps.common import G2BIngestWindow
from apps.g2b_pipeline.app.steps.participation_regions import (
    PARTICIPATION_REGION_URL,
    normalize_participation_region_raw_row,
    participation_region_resource_key,
)


def write_participation_region_raw_records(
    *,
    database_url: str,
    schema_name: str,
    table_name: str,
            ...
            for record in records:
                cursor.execute(
                    ...
                    (
                        ...
                        PARTICIPATION_REGION_URL,
                        participation_region_resource_key(record),
                        ...
                        json.dumps(
                            {
                                ...
                                "ingest_name": "g2b-bid-participation-region-ingest",
                            },
                            ensure_ascii=False,
                        ),
                        now,
...
def normalize_raw_participation_regions(
    *,
    database_url: str,
    raw_schema: str,
    raw_table: str,
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as conn:
        ...
        written = _write_normalized_rows(
            ...
            schema_name=target_schema,
            table_name=target_table,
            rows=[normalize_participation_region_raw_row(row) for row in rows],
        )
        conn.commit()
...
def _ensure_normalized_table(
    ...
    *,
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            f'''
            CREATE INDEX IF NOT EXISTS "{table_name}_region_idx"
            ON "{schema_name}"."{table_name}" (region_name)
            '''
        )
...
def _write_normalized_rows(
    ...
    *,
    ...
    statement = (
        ...
        "registered_at, limit_group_no, limit_serial_no, region_name, region_code, raw_id, updated_at) "
        ...
        "%(region_name)s, %(region_code)s, %(raw_id)s, %(updated_at)s) "
        "ON CONFLICT (resource_key) DO UPDATE SET "
