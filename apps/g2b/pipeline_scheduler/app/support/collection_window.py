...
from apps.g2b.pipeline_scheduler.app.support.settings import get_settings
from apps.g2b.pipeline_scheduler.app.support.job_keys import build_bid_list_page_job_key, build_bid_list_window_job_key
from apps.g2b.pipeline_scheduler.app.support.quota import get_quota_block
from apps.g2b.pipeline_scheduler.app.support.state import G2bIngestTaskStateStore


def _get_mode() -> str:
    return os.getenv("G2B_INGEST_SERVICE_MODE", "auto").strip().lower()


def _get_backfill_start() -> str:
    value = os.getenv("G2B_INGEST_BACKFILL_START", "").strip()
    if not value:
        raise RuntimeError("G2B_INGEST_BACKFILL_START is required for G2B Ingest service")
    return value


def _get_window_minutes() -> int:
    return int(os.getenv("G2B_INGEST_WINDOW_MINUTES", "1440"))


def _get_incremental_lookback_minutes() -> int:
    return int(os.getenv("G2B_INGEST_INCREMENTAL_LOOKBACK_MINUTES", "120"))


def _get_bid_queue_target() -> int:
    return int(os.getenv("G2B_INGEST_BID_QUEUE_TARGET", "10"))


async def build_bid_list_payload(*, service_name: str, queue_name: str, checkpoint_store) -> dict[str, Any] | None:
    settings = get_settings()
    if await get_quota_block(
        redis_url=settings.redis_url,
        database_url=settings.checkpoint_database_url,
    ) is not None:
        return None

    checkpoint = await checkpoint_store.get(f"service:{service_name}")
    database_url = os.getenv(
        "G2B_INGEST_DATABASE_URL",
        get_checkpoint_database_url(host_default="postgres"),
    )
    state_store = G2bIngestTaskStateStore(database_url=database_url)
    try:
        if await state_store.get_active_quota_block() is not None:
            return None
        if await _is_bid_queue_full(queue_name=queue_name):
            return None
