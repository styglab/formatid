...
from apps.g2b.pipeline_scheduler.app.config import (
    G2B_INGEST_PARTICIPANTS_URL,
    G2B_INGEST_WINNERS_URL,
    GENERIC_API_TABLE,
    GENERIC_FILE_TABLE,
)
from apps.g2b.pipeline_scheduler.app.graph.ingest_graph import Notice
from apps.g2b.pipeline_scheduler.app.support.settings import get_settings as get_g2b_ingest_settings
from apps.g2b.pipeline_scheduler.app.support.job_keys import build_bid_notice_job_key, build_bid_number_job_key
from apps.g2b.pipeline_scheduler.app.support.quota import get_quota_block
from apps.g2b.pipeline_scheduler.app.support.repository import G2bIngestRepository
from apps.g2b.pipeline_scheduler.app.support.state import G2bIngestTaskStateStore


INGEST_API_TASK = "ingest.api.fetch"
INGEST_FILE_TASK = "ingest.file.download"
...
async def enqueue_downstream_generic_tasks(
    *,
    ...
    api_queue_name = get_task_definition(INGEST_API_TASK).queue_name
    file_queue_name = get_task_definition(INGEST_FILE_TASK).queue_name
    repository = G2bIngestRepository(database_url=g2b_ingest_settings.database_url)
    state_store = G2bIngestTaskStateStore(database_url=g2b_ingest_settings.database_url)
    ...
    try:
        ...
        await run_store.record(
            run_name=f"g2b_ingest_bid_{job_type}_enqueue",
            queue_name=file_queue_name if job_type == "attachment" else api_queue_name,
            task_name=INGEST_FILE_TASK if job_type == "attachment" else INGEST_API_TASK,
