...
@flow(name="g2b-success-bid-realtime-ingest")
def g2b_success_bid_realtime_ingest() -> dict[str, Any]:
    lookback_minutes = int(os.getenv("G2B_SUCCESS_BID_REALTIME_LOOKBACK_MINUTES", "180"))
    ...
    with g2b_success_bid_ingest_run_lock() as acquired:
        if not acquired:
            return skipped_by_running_ingest(
                flow="g2b-success-bid-realtime-ingest",
                window=window_dict,
                lock_name=lock_name,
            )
