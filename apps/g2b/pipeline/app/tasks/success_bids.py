...
def write_success_bid_records_value(category: str, window: G2BIngestWindow, records: list[dict[str, Any]]) -> int:
    return write_success_bid_raw_records(
        ...
        table_name=os.getenv("G2B_SUCCESS_BID_RAW_TABLE", "successful_bid_raw"),
        category=category,
        window=window,
        records=records,
    )
...
def normalize_success_bids_once(
    *,
    ...
    return normalize_raw_success_bids_repository(
        ...
        raw_table=os.getenv("G2B_SUCCESS_BID_RAW_TABLE", "successful_bid_raw"),
        target_schema=os.getenv("G2B_NORMALIZED_SCHEMA", "g2b"),
        target_table=os.getenv("G2B_SUCCESS_BID_NORMALIZED_TABLE", "successful_bid"),
