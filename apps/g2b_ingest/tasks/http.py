...
class G2bIngestApiError(RuntimeError):
    pass


class G2bIngestDailyQuotaExceededError(G2bIngestApiError):
    def __init__(
        self,
        *,
...
class G2bIngestApiClient:
    def __init__(self) -> None:
        settings = get_settings()
        if not settings.public_api_key:
            raise G2bIngestApiError("PUBLIC_API_KEY is not configured")
    ...
    async def _request(self, *, url: str, params: dict[str, Any]) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self._timeout, follow_redirects=True) as client:
            ...
            if response.status_code == 429:
                raise G2bIngestDailyQuotaExceededError(
                    message="g2b_ingest api request failed: HTTP 429 daily quota exceeded",
        ...
        if _is_daily_quota_exceeded(result_code=result_code, result_msg=result_msg):
            raise G2bIngestDailyQuotaExceededError(
                message=f"g2b_ingest api daily quota exceeded: resultCode={result_code} resultMsg={result_msg}",
                result_code=result_code,
                result_msg=result_msg,
            )
        if result_code != "00":
            raise G2bIngestApiError(
                f"g2b_ingest api request failed: resultCode={result_code} resultMsg={result_msg}"
            )
        return payload
